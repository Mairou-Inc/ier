...
def save_hackability_server(ip, status, attempts, is_my):
    con = get_connect_db()
    try:
        con.cursor().execute(f"INSERT INTO servers (ip, status, is_my, datetime, attempts) VALUES ('{ip}', '{status}', {str(is_my)}, '{get_iso_datetime_now()}', '{attempts}')")
    except:pass
    con.commit()
    con.close()
...
if __name__ == '__main__':
    init_database()
    print(connect('192.168.0.106', 'mirov', '1423'))
